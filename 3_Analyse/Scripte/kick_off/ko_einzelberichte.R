...
for (x in unique(data.final$group)) {
  ...
  zeit <- unlist(strsplit(x, "/"))[3]

  if (!dir.exists(paste("4_Ergebnisse/Grafiken/kick_off/Einzelberichte", kurs, name, sep = "/"))) {
    dir.create(paste("4_Ergebnisse/Grafiken/kick_off/Einzelberichte", kurs, name, sep = "/"), recursiv = T)
  }

  file <- paste0("4_Ergebnisse/Grafiken/kick_off/Einzelberichte/", x, ".pdf")


  date <- unlist(strsplit(zeit, "_"))[1]
  date <- format.Date(as.Date(date, format = "%Y%m%d"), format = "%d.%m.%Y")

  time <- unlist(strsplit(zeit, "_"))[2]
  ...
  text(5.5, 5, kurs, cex = 1.2)

  text(5.5, 3, paste0("Lehrperson: ", name), cex = 1.2)
  text(5.5, 2, date_time, cex = 1)
