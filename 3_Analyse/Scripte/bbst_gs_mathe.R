lab.kurs <- c(
	"A" = "Berufsbegleitende Studien Mathematik Grundschule A",
	"B" = "Berufsbegleitende Studien Mathematik Grundschule B",
	"C" = "Berufsbegleitende Studien Mathematik Grundschule C",
	"D" = "Berufsbegleitende Studien Mathematik Grundschule D",
	"E" = "Berufsbegleitende Studien Mathematik Grundschule E",
	"F" = "Berufsbegleitende Studien Mathematik Grundschule F",
	"G" = "Berufsbegleitende Studien Mathematik Grundschule G",
	"H" = "Berufsbegleitende Studien Mathematik Grundschule H",
	"I" = "Berufsbegleitende Studien Mathematik Grundschule I")

lev.kurs <- c(
	"Berufsbegleitende Studien Mathematik Grundschule A",
	"Berufsbegleitende Studien Mathematik Grundschule B",
	"Berufsbegleitende Studien Mathematik Grundschule C",
	"Berufsbegleitende Studien Mathematik Grundschule D",
	"Berufsbegleitende Studien Mathematik Grundschule E",
	"Berufsbegleitende Studien Mathematik Grundschule F",
	"Berufsbegleitende Studien Mathematik Grundschule G",
	"Berufsbegleitende Studien Mathematik Grundschule H",
	"Berufsbegleitende Studien Mathematik Grundschule I")


l.fragen.gs_mathe <- c(
  "Das Gesamtkonzept für die bbSt Mathe mit der Aufteilung in die Bereiche Fachvorlesung und Fachdidaktik ist zielführend.",
  "Die Inhalte in den Bereichen Fachdidaktik ist zielführend sind aufeinander abgestimmt.",
  "Meine persönlichen Erwartungen an die Ausbildung werden/wurden erfüllt.")



# Variablen bereinigen
bbst_gs_mathe_c <- bbst_gs_mathe %>%
  select(Prüfung, starts_with("TICKED.")) %>%
  rename_at(vars(starts_with("TICKED.")),
    funs(str_replace(., "TICKED.", "")))


bbst_gs_mathe_1 <- bbst_1.recode(bbst_gs_mathe_c)
...
dir.create("4_Ergebnisse/Grafiken/bbst_gs_mathe", showWarnings = F)

kursname <- "bbSt Ma GS 18/19-1"
kursfile <- "bbst_gs_mathe_1819"

if (length(lev.kurs) > 1) {
	kurse <- c("Mathe", as.vector(unique(na.omit(bbst_gs_mathe_1$kurs))))
} else {
	kurse <- "Mathe"
}
...
for (i in kurse) {
	...
	if (i == "Mathe") {
  	file   <- paste0("4_Ergebnisse/Grafiken/bbst_gs_mathe/", kursfile , ".pdf")
		anzahl <- paste0("Teilnehmende: ", nrow(bbst_gs_mathe_c))
		kursnr <- kursname

		write.csv(bbst_gs_mathe_c,paste0("4_Ergebnisse/Tabellen/", kursfile, ".csv"), na = "")

		if (length(lev.kurs) > 1) {
			plot.kurs <- bbst_gs_mathe_1 %>%
		...
		file   <- paste0("4_Ergebnisse/Grafiken/bbst_gs_mathe/", kursfile, " ", str_sub(i, -1, -1), ".pdf")
		anzahl <- paste0("Teilnehmende: ", nrow(bbst_gs_mathe_1[grepl(i, bbst_gs_mathe_1$kurs), ]))
	  kursnr <- paste(kursname, str_sub(i, -1, -1), sep = " ")
	}



  plot.geschl <- plot.pie(bbst_gs_mathe_1, i, geschl, T,
  	"Geschlecht")

  plot.alter <- plot.pie(bbst_gs_mathe_1, i, alter, T,
  	"Alter")

  plot.fach <- plot.pie(bbst_gs_mathe_1, i, fach, T,
  	"Welche Fachrichtung haben Sie studiert?")

  plot.erfahr <- plot.bar(bbst_gs_mathe_1, i, erfahr, T,
  	"Wie viele Jahre Berufserfahrung haben Sie in der studierten Fachrichtung?")

  plot.querein <- plot.bar(bbst_gs_mathe_1, i, querein, T,
  ...
  plot.aufmerk_ausb <- plot.bar(bbst_gs_mathe_1, i, aufmerk_ausb, T,
  	"Wie wurden Sie auf diese Ausbildungsmöglichkeit aufmerksam?")

  plot.schultyp <- plot.bar(bbst_gs_mathe_1, i, schultyp, T,
  	"An welchem Schultyp sind Sie eingesetzt?")

	plot.klassen <- plot.bar.klassen(bbst_gs_mathe_c, bbst_gs_mathe_1)

  plot.stunden <- plot.bar(bbst_gs_mathe_1, i, stunden, T,
  	"Wie viele Stunden eigenständigen Unterricht erteilen Sie pro Woche?")

  plot.stunden_fach <- plot.bar(bbst_gs_mathe_1, i, stunden_fach, T,
  	"Wie viele Stunden davon unterrichten Sie Ihr anerkanntes Fach?")

  plot.stunden_stud <- plot.bar(bbst_gs_mathe_1, i, stunden_stud, T,
  	"Wie viele Stunden unterrichten Sie bereits in dem Fach,\nwelches Sie gerade in den berufsbegleitenden Studien belegen?")

	plot.einsatz <- plot.bar.einsatz(bbst_gs_mathe_c, bbst_gs_mathe_1)

  plot.leitung <- plot.pie(bbst_gs_mathe_1, i, leitung, T,
  	"Fühlen Sie sich seitens der Schulleitung ausreichend unterstützt?")

  plot.belast <- plot.bar.div(bbst_gs_mathe_1, i, belast,
  	"Wie hoch ist Ihre gefühlte Belastung durch die berufsbegleit. Studien und Schule?",
  	"völlig in Ordnung", "maximale Belastung")

  plot.verein <- plot.bar.div(bbst_gs_mathe_1, i, verein,
  	"Wie gelingt Ihnen die Vereinbarkeit von berufsbegleitenden Studien und Familie?",
  	"sehr gut", "ungenügend")

  plot.quali <- plot.bar.div(bbst_gs_mathe_1, i, quali,
  	"Wie empfanden Sie die Qualität der Beratung im Vorfeld der Ausbildung?",
  	"sehr gut", "ungenügend")

  plot.fehlt <- plot.bar.fehlt(bbst_gs_mathe_c, bbst_gs_mathe_1)


  ### plots for batteries
	# Fragen zu den berufsbegleitenden Studien
	bbst_gs_mathe_fragen <- batterie.recode(bbst_gs_mathe_c, i, "fragen")
	names(bbst_gs_mathe_fragen) <- l.fragen.gs_mathe

	# Fachvorlesung
	bbst_gs_mathe_fv <- batterie.recode(bbst_gs_mathe_c, i, "fv")
	names(bbst_gs_mathe_fv) <- items_11

	# Fachdidaktik
	bbst_gs_mathe_fd <- batterie.recode(bbst_gs_mathe_c, i, "fd")
	names(bbst_gs_mathe_fd) <- items_13


  # Fragen zu den berufsbegleitenden Studien
  plot.fragen <- plot.likert(bbst_gs_mathe_fragen,
  	"Fragen zu den berufsbegleitenden Studien Mathe \nfür die Grundschule")

  # Fachvorlesung
  plot.fv <- plot.likert(bbst_gs_mathe_fv,
  	"Fragen zur Struktur und Ablauf in der Fachvorlesung")

  # Fachdidaktik
  plot.fd <- plot.likert(bbst_gs_mathe_fd,
