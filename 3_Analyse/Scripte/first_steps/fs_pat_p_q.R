...
note_q <- round(mean(data.final_pat[data.final_pat$part == "q" & !grepl("Nein|v0", data.final_pat$frage),"note"], na.rm = T), digits = 1)
note_p <- round(mean(data.final_pat[data.final_pat$part == "p" & !grepl("Nein", data.final_pat$frage),"note"], na.rm = T), digits = 1)
...
# Plot für p_q_pat
plot <- data.final_pat %>% filter(!grepl("^Nein|v0", frage), between(note, 1, 6)) %>%
  ggplot(aes(y = note,
...
file <- paste0("4_Ergebnisse/Grafiken/first_steps/", fs_jahr, "/fs_pat_p_q.pdf")
